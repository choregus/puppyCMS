...
$DIRS_AT_TOP=TRUE;
$SHELL_PRE='';
$BRANDING_HEADER='<h1>puppyCMS Editor</h1>Add, edit and delete pages on your site.';
$BRANDING_FOOTER='Adapted from \'Editor\' by Simon Thorpe - <a href="https://github.com/simon-thorpe/editor">https://github.com/simon-thorpe/editor</a>
<br /><br />
Remember: Save your pages with .txt extension or you will not see them in this list and wont be able to edit them.<br/>
You can also upload .gif, .jpg and .png files here.';
$DEFAULT_DIR='';
?>
<?php
...
! function(a) {
    ...
    var b = "",
        c = {};
    try {
        -1 !== window.location.search.search(new RegExp("[?&]p=([^&$]*)", "i")) && (b = RegExp.$1), b = decodeURIComponent(b)
    } catch (d) {
        alert(d), alert("Path=" + b)
    }
    window.editor = c, c.detectFileMode = function(a) {
            if (/^\/etc\/apache2\//.test(a)) return "apache_conf";
            if (/Dockerfile$/.test(a)) return "dockerfile";
        ...
        }, c.detectFileModeByContent = function(a) {
            return 0 === a.indexOf("#!/bin/sh") ? "sh" : 0 === a.indexOf("#!/bin/bash") ? "sh" : ""
        }, c.save = function(a) {
            jQuery.ajax({
                url: "?",
                type: "post",
                dataType: "json",
                data: {
                    content: c.instance.getValue(),
                    p: b
                },
                success: function() {
                    a ? history.back() : c.isModified && (c.isModified = !1, document.title = document.title.substring(1))
                },
                error: function(a) {
            ...
            getItems: function() {
                var a = localStorage.clipValue;
                return a ? JSON.parse(a) : []
            },
            setItems: function(a) {
                localStorage.clipValue = JSON.stringify(a)
            },
            add: function(a, b) {
                var c = this.getItems();
                c.push({
            ...
            remove: function(a) {
                var b, d = a,
                    e = c.clip.getItems();
                return "object" == typeof d && (d = a.path), (b = e.filter(function(a) {
                    return a.path === d
                })[0]) ? (e.splice(e.indexOf(b), 1), this.setItems(e), !0) : !1
            }
        }, c.del = function(b, c) {
            var d = !1,
                e = a(c).parent("div");
            return 0 === e.length && console.error("DEBUG: Cannot find row in DOM."), a.ajax({
                url: "?",
                method: "post",
                data: {
                    p: b,
                ...
                async: !1,
                success: function(a) {
                    a.success === !0 ? (e.remove(), d = !0) : alert(a.message)
                },
                error: function(a) {
                    alert(a.responseText)
                }
            ...
            c.clip.add(a, "cut"), console.log("Clipboard: " + a)
        }, c.copy = function(a) {
            c.clip.add(a, "copy"), console.log("Clipboard: " + a)
        }, c.paste = function(b, d) {
            var e, f, g;
            b || (b = c.clip.getItems()[0].path), e = c.clip.getItems().filter(function(a) {
                return a.path === b
            })[0], f = prompt("copy" === e.type ? "Copy to:" : "Move to:", e.path.substring(1 + e.path.lastIndexOf("/"))), f && (d += "/" + f, g = {
                source: e.path,
                dest: d
            }, "copy" === e.type ? g.cp = 1 : "cut" === e.type && (g.mv = 1), d && a.ajax({
                url: "?",
                method: "post",
                data: g,
                success: function(a) {
                    a.success === !0 ? (c.clip.remove(b), window.location = window.location) : alert(a.message)
                },
                error: function(a) {
                    alert(a.responseText)
                }
        ...
        function() {
            var d, e, f;
            a("#editor").each(function() {
                a("header.list").hide(), f = c.detectFileMode(b), f || (f = c.detectFileModeByContent(document.getElementById("editor").textContent.trim())), c.instance = ace.edit("editor"), a(this).get(0).hasAttribute("data-readonly") && c.instance.setOptions({
                    readOnly: !0,
                    highlightActiveLine: !1,
                    highlightGutterLine: !1
                }), 
                  
                  c.instance.session.setMode("ace/mode/markdown"),
                  c.instance.getSession().setUseWrapMode(true),
                  c.instance.on("change", function() {
                    c.isModified || (c.isModified = !0, document.title = "*" + document.title)
                }), f ? ("undefined" != typeof console && console.log("Setting editor mode: ace/mode/" + f), c.instance.getSession().setMode("ace/mode/markdown")) : "undefined" != typeof console && console.log("Unsupported editor mode. All available modes here: https://github.com/ajaxorg/ace-builds/tree/master/src-noconflict"), c.instance.commands.addCommand({
                    name: "Save",
                    bindKey: {
                        win: "Ctrl-s",
                        mac: "Command-s"
            ...
            }), window.location.hash && (window.location = window.location.search), d = c.clip.getItems(), e = a("#list"), d.forEach(function(d) {
                var f = a("<a/>").text(d.path),
                    g = a("<a/>").addClass("paste").attr("href", "#"),
                    h = a("<a/>").addClass("remove-clip").attr("href", "#");
                e.append(a("<div/>").addClass("clip").append(f).append(g).append(h)), g.click(function() {
                    return c.paste(d.path, b), !1
                }), h.click(function() {
                    return c.clip.remove(d.path) && a(this).parent().remove(), !1
                })
            }), d.length > 0 && a("#list>div.dir>a.paste").css({
                display: "table-cell"
            })
        }(), a(document).keydown(function(b) {
            115 === b.keyCode && a(".shellButton").click()
        }), a(function() {
            a(".searchForm select").change(function() {
                "Locate Database" === a(this).val() ? a(".searchForm input[type=checkbox]").prop("checked", !1).prop("disabled", !0) : a(".searchForm input[type=checkbox]").prop("disabled", !1)
            }), a(".searchForm button").click(function() {
                var c = a(".searchForm select").val(),
                    d = "?p=" + encodeURIComponent(b).replace(/%2F/g, "/");
                a(".searchForm input[type=checkbox]").prop("checked") && (d += "&r="), ("Filenames" === c || "All" === c) && (d += "&find=" + encodeURIComponent(a(".searchForm input[type=text]").val()).replace(/%2F/g, "/")), ("Content (All Files)" === c || "All" === c) && (d += "&grep=" + encodeURIComponent(a(".searchForm input[type=text]").val()).replace(/%2F/g, "/")), "Content (Code Only)" === c && (d += "&find=^[^.]*$|\\.(php|js|json|..?ss|p?html?|as..?|cs|vb|rb|py|txt|md|xml|xslt?|config)$&grep=" + encodeURIComponent(a(".searchForm input[type=text]").val()).replace(/%2F/g, "/")), "Locate Database" === c && (d += "&locate=" + encodeURIComponent(a(".searchForm input[type=text]").val().replace(/[ ]+/g, ".*")).replace(/%2F/g, "/")), window.location = d
            }), a(".newButton").click(function() {
                var c = prompt("New file (end with / for dir):", "new.txt"),
                    d = "file";
                "/" === c.substring(c.length - 1) && (d = "dir", c = c.substring(0, c.length - 1)), c && a.ajax({
                    url: "?",
                    method: "post",
                    data: {
                        "new": 1,
                    ...
                    dataType: "json",
                    success: function(a) {
                        a.success === !0 ? window.location = "?p=" + encodeURIComponent(b + "/" + c).replace(/%2F/g, "/") : alert(a.message)
                    },
                    error: function(a) {
                        alert(a.responseText)
                    }
                ...
                a(this).addClass("active"), a(".searchForm").show().find("input[type=text]:first").select()
            }), a(".shellButton").click(function() {
                a(this).addClass("active");
                var c, d = a(".shellForm").show().find("input[type=text]:first").select(),
                    e = !0,
                    f = !1,
                    g = !0;
                c = function(h) {
                    "undefined" == typeof h && (h = null), a.ajax({
                        url: "?",
                        type: "post",
                        dataType: "json",
                        data: {
                        ...
                        success: function(b) {
                            return b.lastCmd && d.val(b.lastCmd).prop("readonly", !0), b.idle === !0 ? void d.prop("readonly", !1) : (e === !0 && b.first !== !0 && (f = !0, a(".shellForm input#shellFormBackground").prop("checked", !0)), e = !1, g = !g, a("#shellOutput").html((f ? "Continuing last session...\n\n" : "") + b.output + (b["continue"] === !0 ? g ? "_" : " " : "")), void(b["continue"] === !0 ? setTimeout(c, 500) : ("failure" === b.result ? (a(".shellForm").css({
                                backgroundColor: "#f44"
                            }), alert("Last command failed.")) : "success" === b.result ? a(".shellForm").css({
                                backgroundColor: "#6f6"
                            }) : (a(".shellForm").css({
                                backgroundColor: "#bbb"
                            }), alert("Last command had no result.")), window.onbeforeunload = null, d.prop("readonly", !1).select())))
                        }
                    })
                }, c(), a(".shellForm").submit(function() {
                    var b = JSON.parse(localStorage.shellHistory || "[]");
                    return b.push(d.val()), localStorage.shellHistory = JSON.stringify(b), setTimeout(function() {
                        window.onbeforeunload = function() {
                            return "You have a shell command running."
                        }
                    }, 500), a(".shellForm input#shellFormBackground").prop("checked") ? (a(".shellForm").css({
                        backgroundColor: "#fff"
                    }), c(d.val()), !1) : !0
                })
            }), a('#list .dir a[class!="seg"], #list .file a[class!="seg"]').click(function(a) {
                a.stopPropagation()
            }), a("#list .dir, #list .file").click(function() {
                a(this).find("a.seg:last").get(0).click()
            });
            var c = function() {
                var b, c, d = a("#shellHistory"),
                    e = JSON.parse(localStorage.shellHistory || "[]");
                for (b = 0; b < e.length, c = e[b]; b += 1) d.append(a("<option/>").attr("value", c))
            };
            c();
            var d = null,
                e = function(b) {
                    null === d && (d = a('<dialog id="styledModal" />').text("Uploading ").append(a("<progress max=100/>")), a(document.body).append(d), d.get(0).showModal());
                    var c = d.find("progress").get(0);
                    c.value = b, 100 === b && (alert("Upload Complete!"), window.location = window.location)
                };
            a(document.body).dropzone({
                url: "?p=" + b,
                clickable: !1,
                previewsContainer: "body",
                totaluploadprogress: e
            }), a(".uploadButton").dropzone({
                url: "?p=" + b,
                clickable: !0,
                previewsContainer: "body",
                totaluploadprogress: e
...
<?php
	...
	function renderEditor()
	{
		...
		$html.="<button onclick=window.history.back();>Abandon</button>";
		$html.="</nav></header>";
	...
	function renderFileList()
	{
		...
		foreach($files as $filePath)
		{
			...
			for($i=0;$i<$segsCount;$i++){
				$segAppend.='/'.$segs[$i];
				if($i===$segsCount-1 && ($size>=1048576 || preg_match('/\.(php|mp3|aac|ogg|wav|mid|jpg|bmp|gif|png|webp|webm|mp4|mkv|m4v|avi|pdf|zip|rar|tar|gz|7z)$/i',$rFile)===1))
			...
			if (strpos($rFile, 'txt') !== false || strpos($rFile, 'png') !== false || strpos($rFile, 'jpg') !== false  || strpos($rFile, 'gif') !== false  ) { # only show text and image files
			$html.='<div class="'.($isDir?'dir':'file').($size===FALSE?' bad':'').'"><div class=filepath>'.$segAs."</div><span class=size>$friendlySize</span><a href=# class=del $delOnclick></a></div>\r\n";	
			}
